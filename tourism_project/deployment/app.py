...
typeOfContact = st.selectbox("Type of Contact",["Self Enquiry","Company Invited"],index=0)
...
numberOfFollowUps = st.number_input("Number of Follow Ups", min_value=0, max_value=7, value=1)
productPitched = st.selectbox("Product Pitched",['Deluxe', 'Basic', 'Standard', 'Super Deluxe', 'King'],index=2)
preferredPropertyStar = st.selectbox("Preferred Property Star",[3, 4, 5],index=0)
maritalStatus = st.selectbox("Marital Status",['Single', 'Divorced', 'Married', 'Unmarried'],index=2) 
...
pitchSatisfactionScore = st.selectbox("Pitch Satisfaction Score",[1, 2, 3, 4, 5],index=4)
ownCar = st.checkbox("Owns a Car?",value=True)  # convert to int in pandas
numberOfChildren = st.number_input("Number of Children Visiting", min_value=0, max_value=4,value=0)
designation = st.selectbox("Designation",['Manager', 'Executive', 'Senior Manager', 'AVP', 'VP'],index=2)
